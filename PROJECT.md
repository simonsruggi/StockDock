...
- **Price alerts (one-shot)**: 6 conditions (price above/below a threshold, daily change % up/down, near the 52w high/low); created from the watchlist context menu, managed in Settings — grouped by symbol, then by condition (Above/Below…), each level collapsible only when it holds more than one alert; a group switch re-arms/disarms the whole Above/Below group; right-click an alert → Edit… / Duplicate… / Delete (reuses `PriceAlertSheet` modes); re-arm, "triggered" badge, **"Clear all"** with confirmation; local macOS notification via `UNUserNotificationCenter`; evaluated on every price update (WSS tick, REST, launch, wake); fire once, then disarm
...
- **Localisation**: the window's strings live in the 7 `Localizable.strings` files (92 keys added in 1.9.0) alongside the popover's.
