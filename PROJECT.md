...
- **Overview** (`PortfolioOverview.swift`): hero full-bleed col grafico valore come sfondo della card + `SegmentedRangePicker` 1M/3M/6M/ALL + `contentTransition(.numericText())`; stat tiles; **Allocation** (donut con conteggio asset + legenda + strip diversificazione per tipo, hover evidenzia il settore); **movers** con gauge; **posizioni** con weight-bar e hover. Il grafico valore usa gli **snapshot reali** quando ≥2 giorni, altrimenti un **backfill "ESTIMATED"** (linea tratteggiata grigia) ricostruito da storico prezzi × posizioni correnti (`Models/PortfolioBackfill.swift`, puro+testato). La curva stimata **parte dall'acquisto più vecchio** e ogni posizione entra dalla **sua** `purchaseDate` (holding senza data = valorizzate su tutta la finestra): senza questo, "All" proiettava le quantità di oggi fino alla prima quotazione disponibile su Yahoo e disegnava un portafoglio del 2026 a partire dal 2005. Di conseguenza "All" usa la serie **daily 2y** quando l'acquisto più vecchio ci sta dentro (la mensile `range=max` solo per portafogli più vecchi), e l'etichetta dell'asse X si adatta allo **span disegnato** — `6 lug` sotto l'anno, `gen 2005` con anno a 4 cifre sopra (`gen 05` si leggeva come il 5 gennaio).
...
- **Watchlist wide** (`WatchlistWideView.swift`): lista custom ordinabile (header cliccabili) dentro una card. Colonne **Symbol / Name / Price / After hrs / Trend 1M / 52-week**. Ogni prezzo è accoppiato alla **sua** % nella stessa cella e con la stessa baseline (Price → variazione del giorno vs chiusura precedente; After hrs → movimento pre/post vs chiusura regular), così prezzo e % concordano sempre — niente più pill "Change" staccato. **Gerarchia session-aware** (`extendedSession`): in pre/post-market la colonna After hrs prende la precedenza (prezzo ink + `ChangePill` colorato) e Price scende a contesto (grigio); a mercato aperto è il contrario. Gli header **ordinano per la % che vedi** — Price per la % del giorno, After hrs per il movimento pre/post (mai per il prezzo esteso assoluto), via helper puro `StorageService.sortedByExtendedPercent(_:ascending:percent:)` con le righe senza dati estesi sempre in coda. La colonna After hrs (e il suo sort) rispetta il toggle **Show extended hours** delle impostazioni: OFF ⇒ colonna nascosta, restano solo i valori di mercato regular. **Sparkline in un'unica richiesta** batch (`ensureSparklines`, endpoint Yahoo `spark` multi-simbolo). Doppio click o tasto destro → "View Chart".
...
- **Costo CPU (regola)**: pubblicare i tick invalida *tutte* le view che osservano `StockService` (è un `ObservableObject`, granularità = intero oggetto), quindi ogni flush costa un layout + rasterizzazione completi della finestra aperta (~300ms di CPU con la finestra Portfolio). Da qui due regole: (1) qualsiasi animazione legata al prezzo deve chiudersi **dentro** l'intervallo di flush — usare `DS.tick`, mai spring `response` ≥ 0.4, altrimenti SwiftUI interpola senza sosta e ri-rasterizza i blur delle ombre su CPU; (2) il flush si adatta al focus (`tickFlushActive` / `tickFlushBackground` in `AppDelegate`). Profilare con `/usr/bin/sample <pid>`: se compare `RBInterpolatedDisplayListContents` in continuo, c'è un'animazione che non si assesta. Fix strutturale ancora aperto: migrare `StockService` a `@Observable` per invalidare solo le view che leggono la quota cambiata.
